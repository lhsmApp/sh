...
public class GenerateTransferData {
	...
	public String generateVoucherData(List<TableColumns> tableColumns, List<PageData> listTransferData, String deptCode,
		...
		table.addAttribute("id", "T_"+tableCode);// 加入id属性，值为表名,在FMIS中为了区别FMIS表与其他系统表，其他系统表名要以"T_"开头，所以此处值为T_+AMIS表名
		...
		//普通列集合
		Element colS = colsAll.addElement("ColS");// 加入Keys节点,<ColS> 中定义普通列集合
		for (TableColumns tableColumn : tableColumns) {
			if (tableColumn.getColumn_key() != null && tableColumn.getColumn_key().trim().equals("PRI")) {
				...
				key.addAttribute("id", tableColumn.getColumn_name());// 加入id属性，值为列名
				if (tableColumn.getData_type() != null && (tableColumn.getData_type().trim().equals("DECIMAL")
						|| tableColumn.getData_type().trim().equals("DOUBLE") || tableColumn.getData_type().trim().equals("INT")
				...
				col.addAttribute("id", tableColumn.getColumn_name());// 加入id属性，值为列名
				if (tableColumn.getData_type() != null && (tableColumn.getData_type().trim().equals("DECIMAL")
						|| tableColumn.getData_type().trim().equals("DOUBLE") || tableColumn.getData_type().trim().equals("INT")
						|| tableColumn.getData_type().trim().equals("FLOAT"))) {
		...
		//<rows> 标签中可存放多行数据，每行数据存放到<row> 标签中，<KeyValue> 中存放主键值，<ColValue> 存放普通列值。
		Element rows = table.addElement("rows");// 加入rows节点,<rows> 标签中可存放多行数据
		for (PageData transferData : listTransferData) {
			Element row = rows.addElement("row");// 加入rows节点,每行数据存放到<row> 标签中
			// 主键列值集合
