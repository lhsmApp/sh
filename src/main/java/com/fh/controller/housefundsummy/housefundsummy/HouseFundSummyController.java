...
import com.fh.entity.HouseFundSummyModel;
...
public class HouseFundSummyController extends BaseController {
	...
	public @ResponseBody CommonBase report() throws Exception{
        ...
        /*List<HouseFundSummyModel> listData = (List<HouseFundSummyModel>) JSONArray.toCollection(array,HouseFundSummyModel.class);
        List<SysSealed> listTransfer = new ArrayList<SysSealed>();
        if(null != listData && listData.size() > 0){
        	for(HouseFundSummyModel summy : listData){
    			SysSealed item = new SysSealed();
	...
	public @ResponseBody CommonBase summaryDepartString() throws Exception{
        ...
        if(null != listDepart && listDepart.length > 0){
        	for(String depart : listDepart){
    			...
    			itemBefore.setBILL_TYPE(BillType.GOLD_DETAIL.getNameKey().toString());// 枚举  1工资明细,2工资汇总,3公积金明细,4公积金汇总,5社保明细,6社保汇总,7工资接口,8公积金接口,9社保接口
    			...
    			List<PageData> getHaveDate = housefundsummyService.getHave(mapHave);
    			//获取单位重新汇总信息
        ...
        if(commonBase.getCode() == -1){
			housefundsummyService.summaryModelList(listTo, pdBillNum);
			commonBase.setCode(0);
        }
		return commonBase;
	}
